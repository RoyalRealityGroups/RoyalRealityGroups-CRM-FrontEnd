...
import { Save as SaveIcon, Delete as DeleteIcon, Upload as UploadIcon, AttachFile as AttachFileIcon, Add as AddIcon } from '@mui/icons-material';
...
import { retailerApi, countryApi, stateApi, districtApi, mandalApi, cityApi, areaApi } from '../../../api/masters.api';
import { API_ENDPOINTS } from '../../../utils/constants';
import apiClient from '../../../api/axios.config';
import HomeIcon from '@mui/icons-material/Home';
import FolderIcon from '@mui/icons-material/Folder';
import StorefrontIcon from '@mui/icons-material/Storefront';
import type { RetailerFormData } from '../../../types/masters.types';
...
const RetailerForm: React.FC = () => {
  ...
  usePageTitle(isEditMode ? 'Edit Retailer' : 'Add Retailer');
  ...
  const [selectedOutletType, setSelectedOutletType] = useState<DropdownOption | null>(null);
  ...
  // User account state (create mode only)
  ...
  useEffect(() => {
    setBreadcrumbs([
      ...
      { label: 'Retailer', path: '/masters/retailer', icon: <StorefrontIcon fontSize="small" /> },
      { label: isEditMode ? 'Edit Retailer' : 'Add Retailer' },
    ]);
  ...
  // Fetch retailer data for edit mode
  const { data: retailerData, isLoading: isLoadingRetailer } = useQuery({
  ...
  const { data: attachmentsData, refetch: refetchAttachments } = useQuery({
    queryKey: ['retailerAttachments', id],
    queryFn: async () => {
      const response = await apiClient.get(`/api/masters/retailers/${id}/attachments/`);
  ...
  const { data: contactsData } = useQuery({
    queryKey: ['retailerContacts', id],
    queryFn: () => retailerApi.getContacts(id!),
    enabled: isEditMode,
  });
  ...
  const createContactMutation = useMutation({
    mutationFn: (data: any) => retailerApi.createContact(id!, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['retailerContacts', id] });
  ...
  const updateContactMutation = useMutation({
    mutationFn: ({ contactId, data }: { contactId: string; data: any }) =>
      retailerApi.updateContact(id!, contactId, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['retailerContacts', id] });
  ...
  const deleteContactMutation = useMutation({
    mutationFn: (contactId: string) => retailerApi.deleteContact(id!, contactId),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['retailerContacts', id] });
  ...
  const createCityMutation = useMutation({
    ...
    onSuccess: (newCity) => {
      ...
      setValue('city_id', String(newCity.id));
  ...
  const createAreaMutation = useMutation({
    ...
    onSuccess: (newArea) => {
      ...
      setValue('area_id', String(newArea.id));
  ...
  const createShippingCountryMutation = useMutation({
    ...
    onSuccess: (newCountry) => {
      ...
      setValue('shipping_country_id', String(newCountry.id));
  ...
  const createShippingStateMutation = useMutation({
    ...
    onSuccess: (newState) => {
      ...
      setValue('shipping_state_id', String(newState.id));
  ...
  const createShippingDistrictMutation = useMutation({
    ...
    onSuccess: (newDistrict) => {
      ...
      setValue('shipping_district_id', String(newDistrict.id));
  ...
  const createShippingMandalMutation = useMutation({
    ...
    onSuccess: (newMandal) => {
      ...
      setValue('shipping_mandal_id', String(newMandal.id));
  ...
  const createShippingCityMutation = useMutation({
    ...
    onSuccess: (newCity) => {
      ...
      setValue('shipping_city_id', String(newCity.id));
  ...
  const createShippingAreaMutation = useMutation({
    ...
    onSuccess: (newArea) => {
      ...
      setValue('shipping_area_id', String(newArea.id));
  ...
  // Populate form when retailer data is loaded
  useEffect(() => {
    if (retailerData) {
      const data = retailerData as any;
      reset({
        code: retailerData.code || '',
        name: retailerData.name || '',
        distributor_id: retailerData.distributor || '',
        country_id: data.country_id || retailerData.country || '',
        state_id: data.state_id || retailerData.state || '',
        district_id: data.district_id || retailerData.district || '',
        mandal_id: data.mandal_id || retailerData.mandal || '',
        city_id: data.city_id || retailerData.city || '',
        area_id: data.area_id || retailerData.area || '',
        outlet_type_id: retailerData.outlet_type || '',
        outlet_size: retailerData.outlet_size || '',
        company_id: retailerData.company || '',
        street: retailerData.street || '',
        address: retailerData.address || '',
        pincode: retailerData.pincode || '',
        shipping_same_as_billing: retailerData.shipping_same_as_billing ?? true,
        shipping_country_id: data.shipping_country_id || retailerData.shipping_country || '',
        shipping_state_id: data.shipping_state_id || retailerData.shipping_state || '',
        shipping_district_id: data.shipping_district_id || retailerData.shipping_district || '',
        shipping_mandal_id: data.shipping_mandal_id || retailerData.shipping_mandal || '',
        shipping_city_id: data.shipping_city_id || retailerData.shipping_city || '',
        shipping_area_id: data.shipping_area_id || retailerData.shipping_area || '',
        shipping_street: retailerData.shipping_street || '',
        shipping_address: retailerData.shipping_address || '',
        shipping_pincode: retailerData.shipping_pincode || '',
        gstin: retailerData.gstin || '',
        pan: retailerData.pan || '',
        aadhar: retailerData.aadhar || '',
        bank_account_number: retailerData.bank_account_number || '',
        bank_name: retailerData.bank_name || '',
        bank_branch: retailerData.bank_branch || '',
        bank_ifsc: retailerData.bank_ifsc || '',
        bank_account_type: retailerData.bank_account_type || '',
        google_location: retailerData.google_location || '',
        credit_limit: retailerData.credit_limit ? parseFloat(retailerData.credit_limit) : 0,
        credit_days: retailerData.credit_days || 0,
        is_active: retailerData.is_active,
        effective_from: retailerData.effective_from || '',
        effective_to: retailerData.effective_to || '',
        erp_code: retailerData.erp_code || '',
      });
      setSelectedCountry(
        (data.country_id || retailerData.country) && retailerData.country_name
          ? { id: data.country_id || retailerData.country, name: retailerData.country_name }
          : null
      );
      setSelectedState(
        (data.state_id || retailerData.state) && retailerData.state_name
          ? { id: data.state_id || retailerData.state, name: retailerData.state_name }
          : null
      );
      setSelectedDistrict(
        (data.district_id || retailerData.district) && retailerData.district_name
          ? { id: data.district_id || retailerData.district, name: retailerData.district_name }
          : null
      );
      setSelectedMandal(
        (data.mandal_id || retailerData.mandal) && retailerData.mandal_name
          ? { id: data.mandal_id || retailerData.mandal, name: retailerData.mandal_name }
          : null
      );
      setSelectedCity(
        (data.city_id || retailerData.city) && retailerData.city_name
          ? { id: data.city_id || retailerData.city, name: retailerData.city_name }
          : null
      );
      setSelectedArea(
        (data.area_id || retailerData.area) && retailerData.area_name
          ? { id: data.area_id || retailerData.area, name: retailerData.area_name }
          : null
      );
      setSelectedShippingCountry(
        (data.shipping_country_id || retailerData.shipping_country) && retailerData.shipping_country_name
          ? { id: data.shipping_country_id || retailerData.shipping_country, name: retailerData.shipping_country_name }
          : null
      );
      setSelectedShippingState(
        (data.shipping_state_id || retailerData.shipping_state) && retailerData.shipping_state_name
          ? { id: data.shipping_state_id || retailerData.shipping_state, name: retailerData.shipping_state_name }
          : null
      );
      setSelectedShippingDistrict(
        (data.shipping_district_id || retailerData.shipping_district) && retailerData.shipping_district_name
          ? { id: data.shipping_district_id || retailerData.shipping_district, name: retailerData.shipping_district_name }
          : null
      );
      setSelectedShippingMandal(
        (data.shipping_mandal_id || retailerData.shipping_mandal) && retailerData.shipping_mandal_name
          ? { id: data.shipping_mandal_id || retailerData.shipping_mandal, name: retailerData.shipping_mandal_name }
          : null
      );
      setSelectedShippingCity(
        (data.shipping_city_id || retailerData.shipping_city) && retailerData.shipping_city_name
          ? { id: data.shipping_city_id || retailerData.shipping_city, name: retailerData.shipping_city_name }
          : null
      );
      setSelectedShippingArea(
        (data.shipping_area_id || retailerData.shipping_area) && retailerData.shipping_area_name
          ? { id: data.shipping_area_id || retailerData.shipping_area, name: retailerData.shipping_area_name }
          : null
      );
      setSelectedDistributor(
        retailerData.distributor ? { id: retailerData.distributor, name: retailerData.distributor_name || '' } : null
      );
      setSelectedOutletType(
        retailerData.outlet_type ? { id: retailerData.outlet_type, name: retailerData.outlet_type_name || '' } : null
      );
      setSelectedCompany(
        retailerData.company ? { id: retailerData.company, name: retailerData.company_name || '' } : null
      );
    }
  }, [retailerData, reset]);
  ...
  const createMutation = useMutation({
    mutationFn: retailerApi.createRetailer,
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['retailers'] });
      toastSuccess('Retailer created successfully');
      setTimeout(() => {
        navigate('/masters/retailer');
      }, 1000);
    },
    onError: (error: any) => {
      let errorMessage = 'Failed to create retailer';
  ...
  const updateMutation = useMutation({
    mutationFn: (data: RetailerFormData) => retailerApi.updateRetailer(id!, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['retailers'] });
    ...
    onError: (error: any) => {
      let errorMessage = 'Failed to update retailer';
      if (error.response?.data) {
        const data = error.response.data;
        if (typeof data === 'string') {
          errorMessage = data;
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>, attachmentType: string) => {
    ...
    try {
      await apiClient.post(`/api/masters/retailers/${id}/upload_attachment/`, formData, {
  ...
  const handleDeleteAttachment = async (attachmentId: string) => {
    if (!id) return;
    try {
      await apiClient.delete(`/api/masters/retailers/${id}/attachments/${attachmentId}/`);
  ...
  const handleContactSubmit = async (data: any) => {
    if (isEditMode) {
      ...
      if (editingContact) {
        setTempContacts(prev => prev.map(c => c.id === editingContact.id ? { ...data, id: editingContact.id } : c));
      } else {
        setTempContacts(prev => [...prev, { ...data, id: Date.now().toString() }]);
      }
...
const handleShippingCountryCreate = async (data: any) => {
 await createShippingCountryMutation.mutateAsync(data);
};

const handleShippingStateCreate = async (data: any) => {
 await createShippingStateMutation.mutateAsync(data);
};

const handleShippingDistrictCreate = async (data: any) => {
 await createShippingDistrictMutation.mutateAsync(data);
};

const handleShippingMandalCreate = async (data: any) => {
 await createShippingMandalMutation.mutateAsync(data);
};

const handleShippingCityCreate = async (data: any) => {
 await createShippingCityMutation.mutateAsync(data);
};

const handleShippingAreaCreate = async (data: any) => {
 await createShippingAreaMutation.mutateAsync(data);
};
  ...
  const onSubmit = async (data: RetailerFormData) => {
    ...
    // Add user account fields to payload in create mode
    const submitData = {
      ...data,
      ...(!isEditMode && {
    ...
    try {
      if (isEditMode) {
        ...
        if (hasAttachments) {
          ...
          if (tempContacts.length > 0 && result?.id) {
            for (const contact of tempContacts) {
              await retailerApi.createContact(result.id, contact);
            }
          }
        } else {
          const result = await createMutation.mutateAsync(submitData as any);
          if (tempContacts.length > 0 && result?.id) {
            for (const contact of tempContacts) {
              await retailerApi.createContact(result.id, contact);
            }
  ...
  if (isEditMode && isLoadingRetailer) {
    return (
    ...
    <Box sx={getPageContainerStyles()}>
      ...
      <Box sx={getHeaderSectionStyles()}>
        <Box sx={{ 
          ...
          <ScreenHeader
            title={isEditMode ? 'Edit Retailer' : 'Add Retailer'}
            showBackButton
            onBack={handleBack}
            disableBox
          />
        ...
        <Paper sx={{ p: 3, borderRadius: 0 }}>
          ...
          >
            ...
            <Grid container spacing={3}>
              ...
              {/* Distributor - Only show if enabled, filtered by location */}
                  ...
                  <Controller
                    ...
                    render={({ field: { onChange } }) => (
                      <SearchableDropdown
                        ...
                        additionalFilters={{
                          // When area is selected, filter by area (which includes city-wide distributors)
                          // When only city is selected, filter by city
                          ...(selectedArea 
                        ...
                        placeholder={selectedCity ? (channelConfig?.enforce_channel_hierarchy && channelConfig?.enable_distributor ? "Select a distributor" : "Select a distributor (optional)") : "Select city first"}
                      />
                    )}
                  />
                </Grid>
              ...
              <Grid size={{ xs: 12, sm: 6, md: 4 }}>
                <Typography 
                  ...
                  sx={{ mb: 0.5, fontWeight: 600, color: 'text.primary' }}
                >
                  Outlet Type <Box component="span" sx={{ color: '#f44336', fontWeight: 600 }}>*</Box>
                </Typography>
                <Controller
                  name="outlet_type_id"
                  control={control}
                  rules={{ required: 'Outlet type is required' }}
                  render={({ field, fieldState: { error } }) => (
                    <SearchableDropdown
                      ...
                      onChange={(option) => {
                        ...
                        setSelectedOutletType(singleOption);
                        field.onChange(singleOption?.id || '');
                      }}
                      disabled={isSubmitting}
                      error={!!error}
            ...
            {!isEditMode && (
              <>
                ...
                <Grid container spacing={3}>
                  <Grid size={{ xs: 12, sm: 6, md: 4 }}>
                    <Typography variant="subtitle2" sx={{ mb: 0.5, fontWeight: 600, color: 'text.primary' }}>Username <Box component="span" sx={{ color: '#f44336', fontWeight: 600 }}>*</Box></Typography>
                    <TextField value={userUsername} onChange={(e) => { setUserUsername(e.target.value); setUserErrors(prev => { const n = {...prev}; delete n.username; return n; }); }} placeholder="Enter username" fullWidth size="small" disabled={isSubmitting} error={!!userErrors.username} helperText={userErrors.username} />
                  </Grid>
                  <Grid size={{ xs: 12, sm: 6, md: 4 }}>
                  ...
                  <Grid size={{ xs: 12, sm: 6, md: 4 }}>
                    <Typography variant="subtitle2" sx={{ mb: 0.5, fontWeight: 600, color: 'text.primary' }}>Device Access <Box component="span" sx={{ color: '#f44336', fontWeight: 600 }}>*</Box></Typography>
                  ...
                  <Grid size={{ xs: 12 }}>
                    <Typography variant="subtitle2" sx={{ mb: 0.5, fontWeight: 600, color: 'text.primary' }}>Groups <Box component="span" sx={{ color: '#f44336', fontWeight: 600 }}>*</Box></Typography>
...
const RetailerFormWithGuard: React.FC = () => (
  <ChannelPartnerGuard partnerType="retailer">
    <RetailerForm />
  </ChannelPartnerGuard>
);

export default RetailerFormWithGuard;
