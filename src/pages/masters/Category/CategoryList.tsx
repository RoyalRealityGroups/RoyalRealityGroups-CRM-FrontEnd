...
import { categoryApi } from '../../../api/masters.api';
...
import type { CategoryFormData } from '../../../types/masters.types';
...
const CategoryList: React.FC = () => {
  ...
  useEffect(() => {
    setBreadcrumbs([
      ...
      { label: 'Category', icon: <CategoryIcon fontSize="small" /> },
    ]);
  ...
  const { data, isLoading, error, isFetching } = useQuery({
    queryKey: ['categories', paginationModel.page, paginationModel.pageSize, searchQuery],
    queryFn: () =>
  ...
  const handleExport = async () => {
    try {
      setExporting(true);
      await exportApi.genericExport('Category');
      toastSuccess('Export downloaded successfully');
    } catch {
      toastError('Failed to export categories');
    } finally {
      setExporting(false);
    }
  };
  ...
  const columns: GridColDef[] = [
    {
      ...
      align: 'center',
      renderCell: (params) => {
        const index = data?.results.findIndex((row: Category) => row.id === params.row.id) ?? 0;
    ...
    },
    {
      ...
      renderCell: (params) => (
        <Box>
          {hasPermission(user, 'change_category') && (
            <Tooltip title="Edit">
              <IconButton
                size="small"
      ...
      <Box sx={getHeaderSectionStyles()}>
        <Box sx={{ 
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
            ...
            />
            {hasPermission(user, 'export_category') && (
            <Button
              variant="outlined"
            ...
            {hasPermission(user, 'add_category') && (
              <Button
                variant="contained"
                startIcon={<AddIcon />}
                onClick={handleAdd}
