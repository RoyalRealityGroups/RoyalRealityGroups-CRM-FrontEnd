...
import type { SiteVisitFormData, SiteVisitStatus, SiteVisitPhoto } from '../../types/siteVisit.types';
...
const SiteVisitForm: React.FC = () => {
  ...
  const createMutation = useMutation({
    ...
    onError: (err: any) => toastError(err.response?.data?.message || err.response?.data?.status?.[0] || 'Failed to schedule site visit'),
  });
  ...
  const updateMutation = useMutation({
    ...
    onError: (err: any) => toastError(err.response?.data?.message || err.response?.data?.status?.[0] || 'Failed to update site visit'),
  });
  ...
  const handleDeleteSavedPhoto = async (photo: SiteVisitPhoto) => {
    if (!id) return;
    try {
      await siteVisitApi.deletePhoto(id, photo.id);
      setFormData((prev) => ({
        ...prev,
